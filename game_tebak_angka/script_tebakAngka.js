...
const sessionAnswerField = document.getElementById("session-user-answer-field");
...
const localTotalMaximumAttemptsKey = "LOCAL_MAXIMUM_ATTEMPTS";
...
window.addEventListener("load", function () {
  if (typeof Storage !== "undefined") {
    ...
    if (localStorage.getItem(localTotalMaximumAttemptsKey) === null) {
      localStorage.setItem(localTotalMaximumAttemptsKey, 0);
    }
  } else {
  ...
  localMaximumAttemptField.innerText = localStorage.getItem(localTotalMaximumAttemptsKey);
});
...
function updateScore() {
  ...
  const localAttemptsValue = parseInt(localStorage.getItem(localTotalMaximumAttemptsKey));
  if (sessionAttemptsValue > localAttemptsValue) {
    localStorage.setItem(localTotalMaximumAttemptsKey, sessionAttemptsValue);
